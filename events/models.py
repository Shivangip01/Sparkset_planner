...
class Event(models.Model):
    ...
    event_type = models.CharField(max_length=20, choices=TYPE_CHOICES, null=True, blank=True)
...
class VendorProfile(models.Model):
    user = models.OneToOneField(User, on_delete=models.CASCADE)  # Links VendorProfile to a User account
...
class Service(models.Model):
    ...
    vendor = models.ForeignKey(User, on_delete=models.CASCADE)  # Links Service to the Vendor
...
class Feedback(models.Model):
    ...
    rating = models.IntegerField(choices=[(i, str(i)) for i in range(1, 6)], help_text="Rating (1 to 5)")
